...
function xmldb_vpl_upgrade($oldversion = 0) {
    ...
    $vpl44 = 2026050400;
    if ($oldversion < $vpl44) {
        xmldb_vpl_upgrade_2026050400();
        upgrade_mod_savepoint(true, $vpl44, 'vpl');
    }
    return true;
}

/**
 * Adds remote_lab and generate_testbench fields to the vpl table.
 *
 * @return void
 */
function xmldb_vpl_upgrade_2026050400() {
    global $DB;

    $dbman = $DB->get_manager();
    $table = new xmldb_table('vpl');
    $field = new xmldb_field('remote_lab', XMLDB_TYPE_INTEGER, '2', null, XMLDB_NOTNULL, null, '0', 'evaluation_mode');
    xmldb_vpl_addfield($dbman, $table, $field);
    $field = new xmldb_field('generate_testbench', XMLDB_TYPE_INTEGER, '2', null, XMLDB_NOTNULL, null, '0', 'remote_lab');
    xmldb_vpl_addfield($dbman, $table, $field);
}
